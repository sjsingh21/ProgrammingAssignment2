...
makeCacheMatrix <- function(x = matrix()) {
	inverse <- NULL 						## Initialize: Assigning 'NULL' to inverse
	set_matrix <- function(y) {			
		x <<- y 						## SET: Assign the input matrix y to the variable x in the parent environment

		inverse <<- NULL					## Re-initialize m in the parent environment to null

	}
	get_matrix <- function() x 					## GET: Returning matrix 'x'
	set_inverse <- function(solve) inverse <<- solve 	        ## SET: Cache the value of the inverse 
	get_inverse <- function() inverse 				## GET: Returning cached inverse of x
